...
- Prístup: jednotné prihlásenie e-mailom a heslom, cookie session, cieľová obrazovka podľa role a samostatné verejné, rybárske, tímové, kontrolórske, organizačné a prevádzkové priestory. Kontrolór má iba vlastný panel a sektory, brigádnik prevádzkové moduly a účtovník čítacie obchodné podklady.
...
- PWA: manifest, ikony, inštalačný prompt, service worker, offline stránka, stavový banner pripojenia a offline fronty pre rezervácie, úlovky, hlásenia nedostatkov, súťažné hlásenia a kontrolórske admin úkony.
...
- Verejná a interná navigácia sú oddelené; verejný header ukazuje iba verejné moduly a po prihlásení zobrazí jeden rolovo správny vstup do osobného alebo pracovného priestoru.
...
- Rybársky účet: prvý mock e-mailový účet je dostupný v `/konto`. Prihlásený rybár sa serverovo zapíše ako vlastník nového zápisníka cez `ownerUserId`, uvidí svoje aktívne aj ukončené výpravy a ostatní členovia môžu naďalej používať kód bez účtu. Produkčne cookie nahradí Supabase Auth a väzba na `auth.users`.
...
- Neskôr prepojiť čítačku čipov s PWA cez podporovaný hardvér alebo manuálne vloženie čísla čipu.
