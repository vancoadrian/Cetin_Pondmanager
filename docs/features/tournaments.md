...
- `/admin/sutaze` má výber aktívnej súťaže a päť URL pohľadov `Prehľad`, `Prihlášky`, `Sektory`, `Dispečing` a `Pravidlá`. Parameter `turnaj` zachová vybranú súťaž, `sekcia` otvorí konkrétnu pracovnú plochu a všetky prihlášky, hlásenia, váženia, tresty aj kontroly sa filtrujú podľa zvoleného turnaja. Globálny front neodoslaných kontrolórskych úkonov ostáva viditeľný v každom pohľade.
