...
Verejné obrazovky `/zabudnute-heslo` a `/obnova-hesla` slúžia iba lokálne registrovaným rybárom. Žiadosť nikdy nepotvrdí existenciu konkrétneho e-mailu a reset odkaz sa posiela iba cez nakonfigurovaný e-mailový provider.

Prihlásený rybár má v `/konto` súkromný export vlastných údajov. Export nie je dostupný verejnosti ani interným rolám cez tento endpoint a neobsahuje interné poznámky prevádzky či mená ostatných členov spoločnej výpravy.
...
| --- | --- | --- |
| `angler` | `/konto` | vlastné rezervácie podľa e-mailu účtu, zápisníky, výpravy, história úlovkov a nastavenia vrátane zmazania účtu |
