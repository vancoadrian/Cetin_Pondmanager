...
Rybár si môže na verejnej obrazovke `/registracia` vytvoriť vlastný účet. Samoregistrácia vždy prideľuje iba rolu `angler`; interné a súťažné roly naďalej prideľuje prevádzka.

Verejné obrazovky `/zabudnute-heslo` a `/obnova-hesla` slúžia iba rybárskym účtom vrátane predpripraveného rybárskeho účtu; interné a súťažné roly sa nimi nedajú resetovať. Žiadosť nikdy nepotvrdí existenciu konkrétneho e-mailu a reset odkaz sa posiela iba cez nakonfigurovaný e-mailový provider.

Prihlásený rybár má v `/konto` úpravu mena a telefónu aj súkromný export vlastných údajov. Prihlasovací e-mail zostáva bez produkčného overenia zmeny nemenný. Export nie je dostupný verejnosti ani interným rolám cez tento endpoint a neobsahuje interné poznámky prevádzky či mená ostatných členov spoločnej výpravy.
...
| --- | --- | --- |
| `angler` | `/konto` | vlastné rezervácie podľa e-mailu účtu, zápisníky, výpravy, história úlovkov a nastavenia vrátane osobných údajov, exportu, zmeny hesla a zmazania účtu |
