...
Admin kalendár skladá `reservations`, živý closure state a `pegs` do týždňovej mriežky cez helpery v `app/utils/calendar.ts`. Rovnaký closure state používa public mapa, admin mapa, dashboard aj serverová validácia rezervácie.
