...
Kapacitné výpočty mimo samotných miest sú oddelené do utility vrstvy. Požičovňa používa `app/utils/rentals.ts`, ktorý skladá `rentalItems` a `rentalBookings` do vysvetliteľného stavu pre public rezerváciu aj admin sklad. Katalóg položiek a doplnkov má prechodový store `.data/rybolov-cetin/rental-catalog-state.json`, public endpoint vracia iba aktívne položky a admin endpoint vracia celý katalóg na úpravu.
