...
## Runtime perzistencia (Supabase)

Runtime stav aplikácie už nežije na lokálnom filesysteme. Perzistencia má dva explicitné drivery riadené cez `RYBOLOV_STORAGE_DRIVER`:

- `supabase` (predvolený a jediný produkčný): štruktúrované runtime dokumenty v tabuľke `public.runtime_store_states` (jsonb payload + revízia; atomické zápisy cez RPC `runtime_store_upsert` a `runtime_store_compare_and_set`), cookie sessions v tabuľke `public.app_sessions` a binárne assety v privátnych Storage bucketoch `catch-photos`, `sponsor-assets`, `map-assets` a `data-backups`.
- `file`: výhradne explicitný development/testovací adaptér s pôvodným `.data/rybolov-cetin` layoutom (unit testy, offline vývoj). V produkcii je zakázaný — resolvovanie drivera aj boot guard `server/plugins/00.persistence-guard.ts` zlyhajú nahlas; žiadny tichý fallback na filesystem neexistuje.

Prístupová vrstva: `server/utils/runtimeStateStore.ts` (dokumenty; mutácie kombinujú in-process mutex s cross-instance compare-and-set na revízii), `server/utils/assetObjectStore.ts` (objekty) a `server/utils/serverSupabaseClient.ts` (service-role klient — beží iba na serveri a secret kľúč sa nikdy nedostane do klientskeho kódu ani `runtimeConfig.public`). Store moduly `server/utils/local*Store.ts` si ponechali pôvodné kontrakty (validácie, seedovanie, normalizácie) a nad novými primitívami tvoria repository vrstvu; služby a API routes sa nemenili.

`runtime_store_states` aj `app_sessions` majú zapnuté RLS bez policies a bez grantov pre `anon`/`authenticated` — dostupné sú výhradne cez service rolu. Storage buckety sú privátne bez storage policies, takže každý asset tečie len cez serverové endpointy (`/api/catch-photos/:id`, `/api/map-assets/:id`, `/api/sponsor-assets/:id`), ktoré si držia vlastné aplikačné pravidlá. Schéma je v migrácii `supabase/migrations/202608260001_runtime_state_sessions_and_buckets.sql`, prístupové testy v `tests/integration/runtimeStores.test.ts` a jednorazový import legacy `.data` dát rieši idempotentný `pnpm data:import` (bez `--force` nikdy nič neprepíše).
...
Sponzorský modul používa runtime store `sponsor-state`. Public API `/api/sponsors` vracia iba aktívnych partnerov, admin API `/api/admin/sponsors` vracia celý zoznam a `PUT /api/admin/sponsors` zapisuje zmeny vrátane audit udalosti. Logo upload ukladá súbory do privátneho Storage bucketu `sponsor-assets` a verejne ich servuje cez `/api/sponsor-assets/:id`; text loga ostáva fallback. Upload posiela aj šírku a výšku obrázka, takže validácia vie odlíšiť homepage, footer, výsledkovku, sektor a bežnú stránku sponzorov. `getSponsorLogo()` vyberá najprv variant pre konkrétne umiestnenie, potom všeobecný variant a potom hlavné logo. `sponsorPlacements` filtruje aktívnych partnerov pre súťažné sloty `tournament`, `sector` a `scoreboard`; používajú ho verejné súťaže aj admin dispečing. `sponsorLogoVariants` drží jednotné cieľové rozmery a výpočet canvas draw boxu pre hromadné generovanie variantov v admine vrátane X/Y ohniska pri cover oreze; admin UI vie ohnisko nastaviť posuvníkmi alebo priamo v náhľade cez pointer eventy. Zdrojové logo pre generovanie variantov sa dá uložiť ako samostatný lokálny asset a znovu použiť po reloade. Vygenerovaný variant ukladá `cropPreset`, takže lokálny store aj Supabase seed vedia zachovať spôsob vzniku assetu. Štruktúrované polia umiestnenia v prototype pripravujú budúcu tabuľku `sponsor_placements`: typ umiestnenia, poradie, platnosť od-do, súťaž a sektor.

Notifikačná vrstva používa runtime store `notification-state`. Verejná PWA stránka ukladá odbery zariadení cez `/api/notifications/subscribe`, vie ich vypnúť cez `/api/notifications/unsubscribe` a číta aktívne oznamy cez `/api/notifications`. Rybár si pre konkrétne zariadenie vyberá okruhy `weather`, `service`, `reservations`, `tournaments` aj jedno alebo viac jazier; výber sa obnovuje z lokálnej preferencie zariadenia a rovnaký endpoint sa pri uložení aktualizuje bez duplikátu. Klientsky odber používa browser Push API, service worker a verejný VAPID kľúč; ak chýba podpora browsera alebo kľúč, aktuálne oznamy zostávajú čitateľné bez push odberu a stránka zobrazí dôvod. Public subscribe orezáva internú audience vrstvu; mock interné odbery s rolou, turnajom, sektormi a kontrolórom vytvára iba admin endpoint `/api/admin/notifications/subscriptions`. Admin `/admin/notifikacie` pripravuje nový verejný oznam a broadcast cez `/api/admin/notifications/broadcast`; broadcast počíta cieľové odbery podľa okruhov, jazier a voliteľnej internej audience (`roles`, `tournamentId`, `sectorIds`, `marshalIds`), spúšťa delivery provider `mock`, `disabled` alebo `web-push`, zapisuje delivery log po zariadeniach a audit udalosť. Verejný oznam nesie čitateľné `validUntil` aj presné `expiresAt`; public API filtruje skončené alebo ručne ukončené alerty a endpoint `/api/admin/notifications/alerts/:id/end` doplní `endedAt` bez zmazania broadcastu či delivery histórie. Service worker ignoruje oneskorený push po `expiresAt`. Odber bez `lakeIds` a broadcast bez `targetLakeIds` znamenajú všetky jazerá, takže staré lokálne dáta zostávajú kompatibilné; staré broadcasty s platnosťou `dnes ...` dostanú pri načítaní odvodenú expiráciu. Interný endpoint `/api/admin/notifications/test-broadcast` používa rovnaký delivery runner, ale nepridá alert do verejnej stránky výstrah; admin výpisy vedia verejné broadcasty a interné testy filtrovať oddelene vrátane delivery logov a odbery zariadení filtrovať podľa aktívnosti, interného alebo verejného typu a odoberaného okruhu. Endpoint `/api/admin/notifications/test-cleanup` čistí iba staré interné testy podľa retencie, ponechá zvolený počet posledných testov, odstráni ich delivery logy a zapíše audit. Web Push provider používa serverový `web-push` adaptér s VAPID nastavením, TTL, timeoutom a urgenciou z env; admin obrazovka zároveň ukazuje diagnostiku provideru, chýbajúce VAPID premenné a stav pripravenosti reálneho odosielania. VAPID pár sa dá vygenerovať cez `pnpm push:vapid`. Automatické rezervácie, hlásenia nedostatkov, veľké ryby a súťažné udalosti odovzdávajú konkrétne jazero do rovnakého cielenia.
...
Observability vrstva kombinuje privacy-first `@sentry/nuxt` klient/Nitro integráciu s lokálnym development fallbackom. Verejný `/api/health` vracia stav `ok`, `degraded` alebo `down` podľa runtime, dostupnosti dátového úložiska (Supabase databáza a Storage buckety; pri explicitnom file dev adaptéri zapisovateľnosť adresára), environment readiness, pripravenosti notifikačného provideru a počtu posledných chýb. `degraded` s `ok: true` je nefatálne readiness upozornenie; `down` alebo `ok: false` je výpadkový stav. Admin modul `/admin/system` číta detailný `/api/admin/system`, ukazuje rovnaké health checky, aktívny driver dátového úložiska, readiness profil dev/stage/prod, posledné zachytené chyby a súhrn lokálneho data exportu. Lokálny data panel skladá prevádzkový checklist z aktuálneho súhrnu, posledných audit udalostí, import preview a safety backup archívu, aby bolo vidieť stav krokov aktuálny stav → export → kontrola → obnova → archív. Readiness report kontroluje `RYBOLOV_ENVIRONMENT`, verejnú URL, Supabase úložisko (URL, publishable aj secret kľúč, storage driver), Sentry DSN, Web Push/VAPID, Resend reporty, rezervačné e-mailové potvrdenia, cron secret a weather provider podľa aktuálneho profilu. Keď nie je aktívne syntakticky platné browser Sentry DSN, `client-error-reporter.client.ts` zachytáva Vue chyby, `window.error` a `unhandledrejection`, posiela skrátený payload na `/api/client-errors` a server ho ukladá do lokálneho error logu. Pri aktívnom Sentry sa fallback handlery neinštalujú a endpoint nič nezapisuje, aby sa udalosti neduplikovali. Admin endpoint `/api/admin/data-export` skladá jeden JSON backup z runtime store dokumentov a Storage bucketov; `assets=manifest` ponechá súbory ako inventár, `assets=inline` ich vloží do JSON ako base64 pre jednorazový presun. Nové exporty obsahujú `integrity` blok so SHA-256 checksumom kanonického payloadu bez samotného odtlačku. Endpoint `/api/admin/data-import/preview` validuje nahratý backup, overí integritu, porovnáva store s aktuálnym modelom a vracia upozornenia bez akéhokoľvek zápisu do úložiska; staršie backupy bez odtlačku zostávajú použiteľné, ale majú stav integrity `missing`. Endpoint `/api/admin/data-import/restore` vyžaduje frázu `OBNOVIT DATA`, odmietne neplatný alebo iba súhrnný export, pred zápisom vytvorí safety backup aktuálneho stavu v privátnom buckete `data-backups` (file driver: `.data/rybolov-cetin/backups/`) a obnoví iba známe store; inline assety zapíše iba pri `assetPolicy=inline`. Endpointy `/api/admin/data-backups` a `/api/admin/data-backups/:id` listujú a načítajú iba rozpoznané safety backup JSON súbory; reálne stiahnutie používa `?download=1`. Endpoint `/api/admin/data-backups/cleanup` najprv v dry-run režime vráti kandidátov na odstránenie podľa počtu najnovších backupov, ostré čistenie vyžaduje frázu `VYCISTIT BACKUPY` a maže len rozpoznané safety backup súbory. Stiahnutie backupu, preview, úspešný restore, načítanie safety backupu, jeho stiahnutie aj retencia zapisujú systémovú audit udalosť. Runtime store aj assety už bežia na Supabase; observability sa produkčne doplní o nezávislý uptime monitor a databázové zálohy.
...
Admin systémová obrazovka je rozdelená na URL pohľady `Prehľad`, `Dáta` a `Chyby`; parameter `sekcia` otvorí priamo príslušnú pracovnú plochu a globálny health stav s obnovením zostáva dostupný vo všetkých pohľadoch.

Admin audit `/admin/audit` číta najviac 200 posledných udalostí a filtruje ich podľa úplného spoločného zoznamu oblastí, závažnosti, obdobia a textu. Stav filtrov aj otvorený záznam sa uchovávajú v URL parametroch `oblast`, `zavaznost`, `obdobie`, `hladat` a `zaznam`; technické podrobnosti sú predvolene zbalené a aktuálny výber sa dá exportovať do CSV. Rovnaký zoznam oblastí používa serverový `GET /api/admin/audit`, aby nové oblasti účtov, registra rýb a hlásení nedostatkov nezostali mimo filtrovania.

Prevádzkový dashboard `/admin` číta živý stav rezervácií, interných úlovkov, hlásení nedostatkov, privolaní správcu a uzávierok. Namiesto katalógu modulov skladá štyri akčné metriky, zoradenú frontu najviac ôsmich najnaliehavejších úloh, rýchle akcie podľa oprávnení a najbližšie obmedzenia revíru. Každá úloha smeruje na konkrétnu rezerváciu, úlovok alebo hlásenie; hlásenia nedostatkov používajú URL parameter `hlasenie`, ktorý otvorí a odroluje príslušný detail.
...
Kapacitné výpočty mimo samotných miest sú oddelené do utility vrstvy. Požičovňa používa `app/utils/rentals.ts`, ktorý skladá `rentalItems` a `rentalBookings` do vysvetliteľného stavu pre public rezerváciu aj admin sklad. Katalóg položiek a doplnkov má runtime store `rental-catalog-state`, public endpoint vracia iba aktívne položky a admin endpoint vracia celý katalóg na vytváranie a úpravu.

Public prehľad, mapa a rezervácia zdieľajú termín dostupnosti cez URL query parametre `od` a `do`; voliteľné `jazero` a `miesto` zachovávajú kontext pri prechode medzi obrazovkami. Normalizáciu rozsahu a rýchle voľby rieši `app/utils/availabilityDateRange.ts`, spoločné ovládanie `AvailabilityRangePicker` a samotný stav miest počíta `app/utils/availability.ts`. Picker podporuje voliteľný kompaktný mobilný režim, ktorý používa verejná mapa bez zmeny správania ostatných obrazoviek. Parameter `volne=1` je mapový prezentačný filter a nemení rezervačné pravidlá.
...
Obrazovka `/admin/rezervacie` ju používa cez `useAdminReservationWorkflow()`. Stav rezervácií a výpožičiek sa číta a zapisuje cez runtime store `reservation-state`. Rozhodnutie pripraví `communicationDraft` a samostatný `communicationDelivery` výsledok; provider má režimy `mock`, `resend` a `disabled`, pričom `resend` volá rovnaký Resend endpoint a API kľúč ako reporty, ale má vlastný odosielateľský env. Nová webová žiadosť zároveň spúšťa interný reservation broadcast pre owner, manager a worker odbery; broadcast sa nepublikuje do verejných výstrah a Web Push payload otvára admin detail podľa query `rezervacia`.
...
- `POST /api/auth/login` overí mock e-mail a heslo na serveri a odmietne účet evidovaný ako zmazaný.
...
- `GET /api/admin/system` vracia interný health detail, aktívne úložisko a posledné error log záznamy.
- `GET /api/reservations` vracia aktuálny lokálny stav rezervácií a výpožičiek.
- `POST /api/reservations` prijíma verejnú žiadosť, overí ju cez `reservationApiService`, uloží pending rezerváciu s výpožičkami a pripraví interný push broadcast pre prevádzku.
- `GET /api/account/reservations` vyžaduje rybársku session a vracia iba rezervácie priradené k e-mailu účtu alebo staršie záznamy bez e-mailu podľa mena; odpoveď neobsahuje interné poznámky správcu.
- `GET /api/admin/reservations/notifications` vracia čítací súhrn interných push broadcastov k rezerváciám podľa `requestId`, chránený rezervačnými právami.
- `POST /api/admin/reservations/:id/decision` prijíma admin rozhodnutie, používa rovnaký workflow service ako admin obrazovka, zapisuje výsledok a vracia stav prípravy alebo odoslania správy hosťovi.
...
- `GET /api/catch-photos/:id` vráti binárnu fotku z privátneho Storage bucketu `catch-photos`.
...
- `GET /api/admin/audit` vracia posledné audit udalosti pre internú obrazovku `/admin/audit`; voliteľné `area` a `limit` používajú spoločný zoznam všetkých auditných oblastí.

Admin endpointy sú chránené mock access guardom podľa rovnakej matrix ako UI. Kontrakty endpointov sa prechodom na Supabase repository vrstvu nemenili.
...
1. Presunúť klientske čítania na publishable-key Supabase repositories s RLS (normalizované tabuľky z core migrácie).
2. Nahradiť aplikačný scrypt/cookie login Supabase Auth (potvrdenie e-mailu, revokácia sessions, väzba na `auth.users`).
3. Nastaviť produkčné VAPID kľúče a otestovať reálne browser push endpointy.
4. Zapnúť produkčný Supabase projekt: aplikovať migrácie, spustiť `pnpm data:import` proti produkčným dátam a nastaviť zálohy databázy.
