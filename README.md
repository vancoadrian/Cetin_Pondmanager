...
Systémový monitoring má verejný `GET /api/health`, admin detail `/api/admin/system` a lokálny error log `.data/rybolov-cetin/error-log.json`. Klientsky runtime reporter posiela skrátené chyby na `POST /api/client-errors`; `/admin/system` zobrazuje health checky, environment readiness, posledné chyby a lokálny data export.
