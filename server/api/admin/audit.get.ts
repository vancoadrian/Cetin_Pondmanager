...
import { auditAreas, filterAuditEvents, type AuditLogResponse } from '~/services/auditLogService'
...
export default defineEventHandler(async (event): Promise<AuditLogResponse> => {
  ...
  const area = typeof query.area === 'string' && auditAreaSet.has(query.area as AuditArea)
