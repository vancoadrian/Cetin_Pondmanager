...
/**
 * Shared across the catch-report service modules (generation, email,
 * scheduler) so every module returns the same validation-failure shape.
 */
...
/** Shared with catchReportEmailService for delivery-log and attachment ids. */
...
/** Shared with catchReportEmailService for attachment file names. */
...
/** Shared with catchReportEmailService for delivery-log ids. */
...
/** Shared with catchReportGenerationService, which clones a report after generating it. */
export function cloneReport(report: CatchSavedReport): CatchSavedReport {
  return {
    ...report,
