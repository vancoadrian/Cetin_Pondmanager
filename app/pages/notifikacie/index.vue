...
const seedAlerts = FALLBACK_PUBLIC_NOTIFICATION_ALERTS
...
const notificationSupportNoticeTone = computed<'error' | 'success' | 'warning'>(() => {
...
function updateNotificationDeviceState(state: PublicNotificationDeviceState) {
  notificationStatus.value = state.permission
  pushSupport.value = state.support
  subscriptionEndpoint.value = state.subscriptionEndpoint
  selectedLakeIds.value = state.lakeIds
  selectedTopics.value = state.topics
}
...
function formatAlertLakes(lakeIds?: LakeSlug[]) {
  ...
  return lakeIds.map((lakeId) => PUBLIC_NOTIFICATION_LAKE_LABELS[lakeId]).join(', ')
}
...
<template>
  <div>
    ...
    <section class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <LazyNotificationSubscriptionPanel
        class="mt-10"
        hydrate-on-visible
        @state-change="updateNotificationDeviceState"
      />
    </section>
  </div>
</template>
